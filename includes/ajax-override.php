		...
		else 
		{
			...
			if ( $RequestType === 'change' ) {
				...
				if ($chg_daytime !== $daytime) {
					$message_lines[__('When', '12-step-meeting-list')] = "<tr><td>When</td><td style='color:red;'>$chg_daytime</td></tr>";  
					$IsChange = true;
				}
				...
				if ( $typesArrayHasChanged === true )  {
					$message_lines[__('Types', '12-step-meeting-list')] = "<tr><td>Types</td><td style='color:red;'>$chg_typesDescStr</td></tr>";
					$IsChange = true;
				}
				...
				if ( $old !==  $new )  {
					...
					if ( ( strcmp( $m_notes, $c_notes ) !== 0) ) {
						$message_lines[__('Notes', '12-step-meeting-list')] = "<tr><td>Notes</td><td style='color:red;'>$chg_notes</td></tr>";  
						$IsChange = true;
					}
				}

				if ( $chg_conference_url !== $meeting->conference_url ) {
					$message_lines[__('Conference URL', '12-step-meeting-list')] = "<tr><td>Conference URL</td><td style='color:red;'>$chg_conference_url</td></tr>";  
					$IsChange = true;
				}
				...
				if ( ( strcmp( $m_notes, $c_notes ) !== 0) ) {
					$message_lines[__('Conference URL Notes', '12-step-meeting-list')] = "<tr><td>Conference URL Notes</td><td style='color:red;'>$chg_conference_url_notes</td></tr>";  
					$IsChange = true;
				}

				if (  $chg_conference_phone !== $meeting->conference_phone )  {
					$message_lines[__('Conference Phone', '12-step-meeting-list')] = "<tr><td>Conference Phone</td><td style='color:red;'>$chg_conference_phone</td></tr>";  
					$IsChange = true;
				}
				...
				if ( ( strcmp( $m_notes, $c_notes ) !== 0) ) {
					$message_lines[__('Conference Phone Notes', '12-step-meeting-list')] = "<tr><td>Conference Phone Notes</td><td style='color:red;'>$chg_conference_phone_notes</td></tr>";  
					$IsChange = true;
				}

				if (  $chg_location !== $meeting->location )  {
					$message_lines[__('Location', '12-step-meeting-list')] = "<tr><td>Location</td><td style='color:red;'>$chg_location</td></tr>";  
					$IsChange = true;
				}

				if (strcmp(rtrim($meeting->formatted_address), rtrim($_POST['formatted_address'])) !== 0) {
					$message_lines[__('Address', '12-step-meeting-list')] = "<tr><td>Address</td><td style='color:red;'>$chg_address</td></tr>";  
					$IsChange = true;
				}
				...
				if ( $chg_region_id != $meeting->region_id) {
					...
					$message_lines[__('Region', '12-step-meeting-list')] = "<tr><td>Region</td><td style='color:red;'>$chg_region</td></tr>"; 
					$IsChange = true;
				}

				if ( $chg_sub_region !== $meeting->sub_region ) {
					$message_lines[__('Sub Region', '12-step-meeting-list')] = "<tr><td>Sub Region</td><td style='color:red;'>$chg_sub_region</td></tr>";  
					$IsChange = true;
				}

				if ( $chg_location_notes !== $meeting->location_notes ) {
					$message_lines[__('Location Notes', '12-step-meeting-list')] = "<tr><td>Location Notes</td><td style='color:red;'>$chg_location_notes</td></tr>";  
					$IsChange = true;
				}

				/* Addition Group Information - when meeting is registered group with an id */
				if ( ( strcmp( $meeting->group, $chg_group ) !== 0) ) {
					$message_lines[__('Group Name', '12-step-meeting-list')] = "<tr><td>Group Name</td><td style='color:red;'>$chg_group</td></tr>";  
					$IsChange = true;
				}
				...
				if ( !empty( $_POST['district'] ) ) {
					...
					if ( strlen($chg_district) > 0  && $meeting->district_id != $chg_district_id ) {					
						$message_lines[__('District', '12-step-meeting-list')] = "<tr><td>District</td><td style='color:red;'>$chg_district</td></tr>"; 
						$IsChange = true;
					}
				}

				if ($meeting->sub_district != $chg_sub_district) {
					$message_lines[__('Sub District', '12-step-meeting-list')] = "<tr><td>Sub District</td><td style='color:red;'>$chg_sub_district</td></tr>";  
					$IsChange = true;
				}

				if ( $meeting->group_notes != $chg_group_notes) {
					$message_lines[__('Group Notes', '12-step-meeting-list')] = "<tr><td>Group Notes</td><td style='color:red;'>$chg_group_notes</td></tr>";  
					$IsChange = true;
				}

				if ( $meeting->website != $chg_website ) {
					$message_lines[__('Website', '12-step-meeting-list')] = "<tr><td>Website</td><td style='color:red;'>$chg_website</td></tr>";  
					$IsChange = true;
				}

				if ( $meeting->website_2 != $chg_website_2 ) {
					$message_lines[__('Website 2', '12-step-meeting-list')] = "<tr><td>Website 2</td><td style='color:red;'>$chg_website_2</td></tr>";  
					$IsChange = true;
				}
			
				if ( $meeting->email != $chg_email ) {
					$message_lines[__('Email', '12-step-meeting-list')] = "<tr><td>Email</td><td style='color:red;'>$chg_email</td></tr>";  
					$IsChange = true;
				}

				if ( $meeting->phone != $chg_phone ) {
					$message_lines[__('Phone', '12-step-meeting-list')] = "<tr><td>Phone</td><td style='color:red;'>$chg_phone</td></tr>";  
					$IsChange = true;
				}

				if ( $meeting->mailing_address != $chg_mailing_address ) {
					$message_lines[__('Mailing Address', '12-step-meeting-list')] = "<tr><td>Mailing Address</td><td style='color:red;'>$chg_mailing_address</td></tr>";  
					$IsChange = true;
				}

				if ( $meeting->venmo != $chg_venmo) {
					$message_lines[__('Venmo', '12-step-meeting-list')] = "<tr><td>Venmo</td><td >$chg_venmo</td></tr>";  
					$IsChange = true;
				}

				if ( $meeting->square != $chg_square) {
					$message_lines[__('Square', '12-step-meeting-list')] = "<tr><td>Square</td><td style='color:red;'>$chg_square</td></tr>";  
					$IsChange = true;
				}

				if ( $meeting->paypal != $chg_paypal) {
					$message_lines[__('Paypal', '12-step-meeting-list')] = "<tr><td>Paypal</td><td style='color:red;'>$chg_paypal</td></tr>";  
					$IsChange = true;
				}

				if ( $meeting->contact_1_name != $chg_contact_1_name ) {
					$message_lines[__('Contact 1 Name', '12-step-meeting-list')] = "<tr><td>Contact 1 Name</td><td style='color:red;'>$chg_contact_1_name</td></tr>";  
					$IsChange = true;
				}

				if ( $meeting->contact_1_email != $chg_contact_1_email ) {
					$message_lines[__('Contact 1 Email', '12-step-meeting-list')] = "<tr><td>Contact 1 Email</td><td style='color:red;'>$chg_contact_1_email</td></tr>";  
					$IsChange = true;
				}

				if ( $meeting->contact_1_phone != $chg_contact_1_phone ) {
					$message_lines[__('Contact 1 Phone', '12-step-meeting-list')] = "<tr><td>Contact 1 Phone</td><td style='color:red;'>$chg_contact_1_phone</td></tr>";  
					$IsChange = true;
				}

				if ( $meeting->contact_2_name != $chg_contact_2_name ) {
					$message_lines[__('Contact 2 Name', '12-step-meeting-list')] = "<tr><td>Contact 2 Name</td><td style='color:red;'>$chg_contact_2_name</td></tr>";  
					$IsChange = true;
				}

				if ( $meeting->contact_2_email != $chg_contact_2_email ) {
					$message_lines[__('Contact 2 Email', '12-step-meeting-list')] = "<tr><td>Contact 2 Email</td><td style='color:red;'>$chg_contact_2_email</td></tr>";  
					$IsChange = true;
				}

				if ( $meeting->contact_2_phone != $chg_contact_2_phone ) {
					$message_lines[__('Contact 2 Phone', '12-step-meeting-list')] = "<tr><td>Contact 2 Phone</td><td >$chg_contact_2_phone</td></tr>";  
					$IsChange = true;
				}

				if ( $meeting->contact_3_name != $chg_contact_3_name ) {
					$message_lines[__('Contact 3 Name', '12-step-meeting-list')] = "<tr><td<td style='color:red;'>Contact 3 Name</td><td style='color:red;'>$chg_contact_3_name</td></tr>";  
					$IsChange = true;
				}

				if ( $meeting->contact_3_email != $chg_contact_3_email ) {
					$message_lines[__('Contact 3 Email', '12-step-meeting-list')] = "<tr><td>Contact 3 Email</td><td style='color:red;'>$chg_contact_3_email</td></tr>";  
					$IsChange = true;
				}

				if ( $meeting->contact_3_phone != $chg_contact_3_phone ) {
					$message_lines[__('Contact 3 Phone', '12-step-meeting-list')] = "<tr><td>Contact 3 Phone</td><td style='color:red;'>$chg_contact_3_phone</td></tr>";  
					$IsChange = true;
				}
